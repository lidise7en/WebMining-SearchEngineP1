 ...
 *  including considering stopwords, field-based retrieval and decision about dashes and slashes 
 *  including deleting terms that are not appeared in indices
 */
		...
		while(i <= this.aft_str.length() - 1)
		{
			...
			if(this.aft_str.charAt(i) == '(')
			{
				if(this.aft_str.charAt(i + 1) != '#')
				{
					...
					File try_read = new File("resource/" + temp + ".inv");// delete the terms that not in indices
				
					try{
						FileInputStream fis = new FileInputStream(try_read);     
			...
			else if(this.aft_str.charAt(i) == ' ')
			{
				...
				if(this.aft_str.charAt(i) == ' ')
				{
					...
					catch(IOException e)
					{
						...
						System.out.println("the after str is" + this.aft_str);
						i =flag;
					}
				}
				else
				{
					...
					{
						delete_substring(flag,i - 1);
						System.out.println("should delete" + this.aft_str.length());
						System.out.println("the after str is" + this.aft_str);
						i =flag;
					}
				}
